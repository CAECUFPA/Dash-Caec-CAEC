...
import { Database, Activity, Landmark, ShieldCheck, Clock, Globe } from 'lucide-react';

/**
 * TopBar: Versão Gestão Institucional CAEC
 * Foco: Autoridade financeira, segurança de dados e métricas em tempo real.
 */
export default function TopBar({ recordCount, lastSyncTime }) {
  return (
    ...
    ">
      {/* SEÇÃO ESQUERDA: Identificação Institucional */}
      <div className="flex flex-col gap-5 w-full md:w-auto">
        <div className="flex items-center gap-5">
          <div className="relative group">
            {/* Brilho de autoridade */}
            ...
            <div className="relative bg-[#0a0f1a] p-2.5 rounded-xl border border-atena-yellow/40 shadow-[inset_0_0_10px_rgba(251,191,36,0.1)]">
              {/* Ícone de Landmark (Instituição/Tesouraria) em vez de CPU */}
              <Landmark size={22} className="text-atena-yellow drop-shadow-[0_0_8px_rgba(251,191,36,0.4)]" />
            </div>
          </div>

          <div className="flex flex-col">
            <h2 className="text-2xl font-black uppercase tracking-[-0.05em] text-white sm:text-3xl lg:text-4xl leading-none">
              CONTROLE <span className="text-atena-yellow/90">FINANCEIRO</span>
            </h2>
            <p className="text-[8px] font-black text-slate-500 uppercase tracking-[0.6em] mt-1.5 ml-0.5">
              CAEC • Diretoria de Administração e Financeiro
            </p>
          </div>
        </div>

        <div className="flex flex-wrap items-center gap-4 md:gap-8 ml-0.5">
          {/* BADGE DE SEGURANÇA: Substituindo o Wifi por ShieldCheck */}
          <div className="
          ...
          {/* CONTADOR DE REGISTROS (AUDITORIA) */}
          <div className="flex items-center gap-3 h-4">
            ...
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-[8px] font-bold uppercase tracking-[0.2em] text-slate-500">Lançamentos Auditados</span>
            </div>
          </div>
        </div>
      </div>

      {/* SEÇÃO DIREITA: Telemetria de Atualização */}
      <div className="flex flex-row md:flex-col items-center md:items-end justify-between w-full md:w-auto gap-4 md:gap-3">
        <div className="flex items-center gap-2.5 px-2">
          <Clock size={12} className="text-atena-yellow/70" />
        ...
        <div className="flex items-center gap-4">
          {/* DISPLAY DE HORA - Estilo Terminal de Gestão */}
          <div className="
          ...
          <div className="flex flex-col items-end border-l-2 border-white/5 pl-5 gap-1">
            <span className="text-[8px] font-black text-slate-600 uppercase tracking-widest">Fluxo</span>
            <div className="flex items-center gap-1.5">
              <span className="text-[9px] font-black text-emerald-500 uppercase italic tracking-[0.2em] brightness-125">Ativo</span>
              <div className="w-1.5 h-3 bg-emerald-500/20 rounded-full overflow-hidden">
                <div className="w-full h-full bg-emerald-500 animate-[bounce_2s_infinite]" />
              </div>
            </div>
          </div>
        </div>
      ...
      {/* DETALHE ESTÉTICO: LINHA DE LUZ */}
      <div className="absolute bottom-0 left-0 h-[1.5px] w-full bg-gradient-to-r from-transparent via-atena-yellow/20 to-transparent opacity-20" />
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 h-[1px] w-1/4 bg-atena-yellow shadow-[0_0_15px_rgba(251,191,36,0.6)] opacity-40" />
    </header>
  );
}
